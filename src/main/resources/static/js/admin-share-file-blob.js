function generateShareLink() {
    ...
    if (fileName) {
        const link = window.location.origin + '/api/admin/storage/blobs/share/download?fileName=' + encodeURIComponent(fileName);
...
window.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('uploadForm').addEventListener('submit', function(e) {
        ...
        fetch('/api/admin/storage/blobs/share/upload', {
            method: 'POST',
            body: formData
        })
    ...
    document.getElementById('downloadForm').addEventListener('submit', function(e) {
        ...
        fetch('/api/admin/storage/blobs/share/download?fileName=' + encodeURIComponent(fileName))
            .then(response => {
                if (response.ok) {
    ...
    document.getElementById('listFilesButton').addEventListener('click', function() {
        fetch('/api/admin/storage/blobs/listFilesOfShared')
            .then(response => response.text())
            .then(data => {
                document.getElementById('responseMessage').textContent = data;
            })
