...
<div class="storage-container mt-4">
    <h2 class="text-center admin-theme mb-4">📧 Admin Email Service</h2>
    <p class="text-center text-muted mb-4">Send an official email from <strong>@leaflogic.xyz</strong> directly via your backend.</p>
    ...
    <form method="post" action="/admin-email/send">
        ...
        <div class="mb-3">
            <label for="fromEmail" class="form-label">Sender (From):</label>
            <input type="email" class="form-control" id="fromEmail" name="fromEmail" value="noreply@leaflogic.xyz" required>
            <div class="smtp-note">Must be verified in your MailerSend/Brevo/SendGrid account. OCI Free Tier blocks port 25, so use an SMTP Relay!</div>
        </div>

        <div class="mb-3">
            <label for="toEmail" class="form-label">Recipient (To):</label>
            <input type="email" class="form-control" id="toEmail" name="toEmail" placeholder="user@example.com" required>
        </div>

        <div class="mb-3">
            <label for="subject" class="form-label">Subject:</label>
            <input type="text" class="form-control" id="subject" name="subject" placeholder="Test from SRT Translation..." required>
        </div>

        <div class="mb-3">
            <label for="message" class="form-label">Message:</label>
            <textarea class="form-control" id="message" name="message" rows="5" required>Hello,
This is a test email sent from the leaflogic.xyz admin panel!</textarea>
        </div>

        <button type="submit" class="btn btn-admin">Send Email</button>
    </form>
